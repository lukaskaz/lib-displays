#include "display/interfaces/sevseg/m74hc595/multi/bicolor/display.hpp"
...
struct Display::Handler
{
  public:
    Handler(const std::string& dev, const config_t& config) :
        ...
        dispsize{std::get<uint8_t>(config)}

    {
        ...
        start();
        log(logs::level::info,
            "Created bicolor multi 7segm display w/ shifting "
    ...
    ~Handler()
    {
        stop();
        close(spifd);
        log(logs::level::info, "Released bicolor multi 7segm display");
    }
  ...
  private:
    ...
    {
        using namespace display::sevseg::charmaps::multi::bicolor;
        auto hasdot = (ch & dotflag) != 0;
        auto str = std::string{(char)(ch & ~dotflag)};
        if (charmap.contains(str))
        {
            code = hasdot ? charmap.at(str) | charmap.at(".") : charmap.at(str);
            return true;
        }
        return false;
    }

    std::string getdotmasked(const std::string& input)
    {
        auto text{input};
        auto pos = text.find('.');
        if (pos != std::string::npos && pos > 0 && pos < text.size() - 1)
        {
            text.at(pos - 1) |= dotflag;
            text.erase(pos, 1);
        }
        return text;
    }
    ...
    bool showdigit(uint8_t segnum, uint8_t code) const
    {
        ...
        segnum <<= color == colortype::first ? 0x00 : 0x04;
        // segnum |= segnum << 0x04;
        // segnum = segnum << 0x04;
        segnum = ~segnum;
        ...
        log(logs::level::debug,
            "Code sent to display(" + std::string(ret == 0 ? "ok" : "err") +
                ...
                " -> segnum: " +
                std::bitset<8 * sizeof(uint8_t)>(segnum).to_string());
    ...
    bool showtext(const std::string& text) const
    {
        ...
        {
            for (uint32_t idx{0}; idx < dispsize && idx < text.size(); idx++)
            {
